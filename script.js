...
function setupSimple() {
  ...
  for (let i = 0; i < 128; i++) {
    let node = new Segment(node, 8, 0, 3.14159 / 2, 1);
  }
...
function setupTentacle() {
  ...
  //(parent,size,angle,range,stiffness)
  for (let i = 0; i < 32; i++) {
    let node = new Segment(node, 8, 0, 2, 1);
  }
  //(end,length,speed,creature)
  let tentacle = new LimbSystem(node, 32, 8, critter);
  setInterval(function () {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
...
function setupArm() {
  ...
  //(parent,size,angle,range,stiffness)
  for (let i = 0; i < 3; i++) {
    let node = new Segment(node, 80, 0, 3.1416, 1);
  }
  let tentacle = new LimbSystem(node, 3, critter);
  setInterval(function () {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    critter.follow(canvas.width / 2, canvas.height / 2);
  }, 33);
...
function setupTestSquid(size, legs) {
  ...
  for (let i = 0; i < legNum; i++) {
    let node = critter;
    let ang = (Math.PI / 2) * (i / (legNum - 1) - 0.5);
    ...
    let leg = new LegSystem(node, jointNum, size * 30, critter);
  }
  setInterval(function () {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    critter.follow(Input.mouse.x, Input.mouse.y);
...
let legNum = Math.floor(1 + Math.random() * 12);
setupLizard(
  8 / Math.sqrt(legNum),
  legNum,
